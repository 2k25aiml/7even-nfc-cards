...
import { a as Subscribable, s as shallowEqualObjects, h as hashKey, g as getDefaultState, n as notifyManager, b as useQueryClient, r as reactExports, d as noop, e as shouldThrowError, o, v as vt } from "./index-BCepy_kX.js";
...
function useDuplicateOrder() {
  const { actor } = useActor(createActor);
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (sourceOrderId) => {
      if (!actor) throw new Error("Actor not available");
      return actor.duplicateOrder(sourceOrderId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["myOrders"] });
    }
...
function useConfirmOrderPayment() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.confirmOrderPayment(
        orderId,
        intentId
      );
    },
...
export {
  ...
  ue as b,
  useOrders as c,
  useDuplicateOrder as d,
  useConfirmOrderPayment as e,
  useCreateOrder as u
};
